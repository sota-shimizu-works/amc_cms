...
export default function PageEdit() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error(e);
      setError(
        "ページの取得に失敗しました。ネットワーク接続や権限をご確認ください。"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (values: PageFormValues) => {
    try {
      ...
      toast.success("更新完了", { description: "ページを更新しました。" });
      router.push(`/test/${id}`);
    } catch (e) {
      toast.error("通信エラー", {
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success("削除完了", { description: "ページを削除しました。" });
      router.push("/test");
    } catch {
    ...
    <>
      <div className="mb-4 flex justify-end">
        <DeleteConfirmButton
          ...
          title="削除"
          description="この検査を本当に削除しますか？この操作は取り消せません。"
          triggerLabel="検査を削除"
