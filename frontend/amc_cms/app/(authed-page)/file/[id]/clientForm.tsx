...
export default function PageEdit() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("メディアの取得に失敗しました");
      const data = await res.json();
      setInitialValues(data);
    } catch (e) {
      console.error(e);
      setError(
        "メディアの取得に失敗しました。ネットワーク接続や権限をご確認ください。"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (values: PageFormValues) => {
    try {
      ...
      toast.success("更新完了", { description: "メディアを更新しました。" });
      router.push(`/file/${id}`);
    } catch (e) {
      toast.error("通信エラー", {
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success("削除完了", { description: "メディアを削除しました。" });
      router.push("/file");
    } catch {
    ...
    <>
      <div className="mb-4 flex justify-end">
        <DeleteConfirmButton
          ...
          title="削除"
          description="このメディアを本当に削除しますか？この操作は取り消せません。"
          triggerLabel="メディアを削除"
