...
export default function PageEdit() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("保険診療の取得に失敗しました");
      const data = await res.json();
      setInitialValues(data);
    } catch (e) {
      console.error(e);
      setError(
        "保険診療の取得に失敗しました。ネットワーク接続や権限をご確認ください。"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (values: PageFormValues) => {
    try {
      ...
      toast.success("更新完了", { description: "保険診療を更新しました。" });
      router.push(`/insured/${id}`);
    } catch (e) {
      toast.error("通信エラー", {
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`/api/insureds/${id}`, { method: "DELETE" });
      if (!res.ok) {
        const error = await res.json();
        toast.error("削除失敗", {
      ...
      toast.success("削除完了", { description: "保険診療を削除しました。" });
      router.push("/insured");
    } catch {
    ...
    <>
      <div className="mb-4 flex justify-end">
        <DeleteConfirmButton
          ...
          title="削除"
          description="この保険診療を本当に削除しますか？この操作は取り消せません。"
          triggerLabel="保険診療を削除"
